...
class TestFindDevicesLegacy(unittest.TestCase):
    @classmethod
    def setUpClass(self):
        python_ics.override_library_name(r"icsneo40-legacy.dll")
        if _DEBUG:
            import os
    ...
    def test_default_parameters_time(self):
        if not _DEBUG:
            self.assertTrue((timeit.timeit("python_ics.find_devices()", setup="import python_ics", number=1000)) <= 5.0)
    ...
    def test_find_vcan3(self):
        devices = python_ics.find_devices(python_ics.NEODEVICE_VCAN3)
        self.assertTrue(len(devices) == 1)

    def test_find_fire2_and_vcan3(self):
        devices = python_ics.find_devices(python_ics.NEODEVICE_FIRE2 | python_ics.NEODEVICE_VCAN3)
        self.assertTrue(len(devices) == 2)
...
class TestFindDevicesNewStyle(unittest.TestCase):
    @classmethod
    def setUpClass(self):
        python_ics.override_library_name(r"icsneo40.dll")
        if _DEBUG:
            import os
